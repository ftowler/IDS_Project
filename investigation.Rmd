...
results %>%
  ...
  )%>% group_by(season)  %>% drop_na(season) %>% summarise(y =mean(split_time, na.rm = TRUE)) %>%
  ggplot() +
  geom_point(mapping = aes(x = season, y = y))
...
qd <- qd %>% 
  filter(!(raceId >= 18 & raceId <= 336),!(raceId >= 356 & raceId <= 840),!(raceId >= 1097 & raceId <= 1110),!(raceId >= 1048 & raceId <= 1051),!(raceId >= 946 & raceId <= 947),!(raceId >= 919 & raceId <= 925))
qd <- qd %>% 
  filter(q3 != "\\N")
avg_quali <- qd %>%
  select(q3, raceId) %>%
...
timeToSec<- function(x){
  secondsQd<- as.numeric(sub(".*:(\\d+\\.\\d+)", "\\1", x))+60
  return(secondsQd)}
avg_quali$q3<-timeToSec(avg_quali$q3)

splitPerRace <- function(x) {
  p1time <- 10000
  total <- 0
  filtered_data <- avg_quali %>% filter(raceId == x)
  for (y in filtered_data$q3) {
    for (z in filtered_data$q3) {
      if (z < p1time) {
        p1time <- z
      }
      total <- total + (p1time - y)
    }
  }
  mtotal <- mean(total)
  return(mtotal)
...
avg_qualisplit <- data.frame(
  raceid = 1:1096,
  splitTime = sapply(1:1096, function(i) splitPerRace(i))
)
avg_qualisplit <- avg_qualisplit %>%
  filter(splitTime != 0)
raceid_ranges <- list(
  c(1, 17, 2009),
  c(337, 355, 2010),
...
for (range in raceid_ranges) {
  result <- avg_qualisplit %>%
    filter(raceid %in% range[1]:range[2]) %>%
    summarise(year = range[3], mean_splitTime = mean(splitTime))
  
  results_list[[as.character(range[3])]] <- result
}

# Combine the results into a single data frame
splitperseason <- bind_rows(results_list, .id = "year")
```
```{r quali split graph}
splitperseason%>%
    ggplot(aes(x = as.numeric(year), y = mean_splitTime*-1)) +
  geom_point() +
  geom_smooth(method = 'lm', na.rm = TRUE, se = FALSE) +
  labs(
    title = "Average split time of qualifying per season",
    x = "Season",
    y = "Average Split Time",
    subtitle = '2009-2022')
```
